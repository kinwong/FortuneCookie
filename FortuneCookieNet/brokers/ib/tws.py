...
class WhatToShow(Enum):
    ...
    All different kinds of historical data are returned in the form of candlesticks and as such the values return
    represent the state of the market during the period covered by the candlestick. For instance, it might appear
...
class TwsClient(object):
    ...
    def connect(self, host: str="", port: int=7496)->Disconnecting:
        ...
        if not self._socket.eConnect(host, port, self.client_id):
            raise RuntimeError("Client[%d] Failed to connect at Host: %s, Port: %d" % (self._client_id, host, port))

...
import kintools
from brokers.ib.tws import TwsClient, Request, BarSize
...
class HistoricalData(object):
    ...
    def historicalData(self, request: Request, the_date, open_, high, low, close, volume, bar_count, wap, has_gaps):
        # print("Req[%d] Historical=> %s - Open: %s, High: %s, Low: %s, Close: %s, Volume: %d" %
        #       (request.request_id, date, open_, high, low, close, volume))
        row = kintools.Expando()
        row.date = the_date
        row.open = open_
        row.high = high
        row.low = low
...
def store_file(root_dir, tws: TwsClient, the_date: date, contract: Contract):
    ...
    request = None
    while retry > 0:
        request = tws.reqHistoricalData(data, contract, end_date.strftime("%Y%m%d 00:00:00"), bar_size=BarSize.Sec30)
        request.done.wait(timeout=REQUEST_TIME_OUT)
        if request.error is not None:
            print('Error caught from request [%d] wait %d seconds then...' % (request.request_id, RETRY_WAIT))
            time.sleep(RETRY_WAIT)
            retry -= 1
            if retry == 0:
                break
            else:
                print('Retry fetching symbol "%s" again - %d try remaining...' % (contract.symbol, retry))
                continue  # try again
    ...
    if request.done.is_set():
        ...
        with open(full_path, 'w', newline='') as csvfile:
                ...
                csv_writer.writerow(['date', 'open', 'high', 'low', 'close', 'volume', 'bar_count', 'wap', 'has_gaps'])
                for row in data.rows:
                    csv_writer.writerow([row.date, row.open, row.high, row.low, row.close, row.volume, row.bar_count,
                                         row.wap, row.has_gaps])
...
def data_date_range():
    return filter(lambda d: d.weekday() not in [5, 6],
                  kintools.date_range(DATA_START, DATA_END))
